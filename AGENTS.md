...
- `pnpm lint` — eslint flat config (`eslint.config.mjs`). Repo currently has 0 errors / ~27 warnings; don't add new ones.
...
- `src/proxy.ts` (Clerk 7 uses `proxy.ts` at src root, not `middleware.ts`) composes `clerkMiddleware` + next-intl middleware with `localePrefix: "always"`. Protected routes: `/twig(.*)`, `/leaves(.*)`.
- Backend is Convex in `convex/`: tables `twigs`, `leaves`, `completions` (`convex/schema.ts`). Client code uses `useQuery`/`useMutation` from `convex/react` with `api.twigs.*` / `api.leaves.*`. Auth is Clerk-wired via `ConvexProviderWithClerk` in `src/app/providers.tsx`.
...
The product was refactored to tree vocabulary and is mid-transition: `calendars` → **twigs**, `habits` → **leaves**. Planned: `trunk`, `limbs`, `branches` (some placeholder pages exist). Always use this vocabulary in UI text, routes, and component names.
